...
class Solution(SolutionBase):
    ...
    def act(self, env: FinalEnv, current_timestep: int):
        ...
        # phase 0: initialize to some poses
        if self.phase == 0:
        ...
        if self.phase == 2:
            ...
            if self.counter == 2000 / 5:
                self.phase = 3
                #move the spade up, keep it parallel to ground

                pose = r2.get_observation()[2][9]
                p, q = pose.p, pose.q
                p[2] += 0.2
                q = euler2quat(np.pi, -np.pi / 0.75, np.pi / 2)
        ...
        if self.phase == 3:
            print("Phase 3")
            # self.freeze(c4, 'start to move up.')
            self.counter += 1
            self.diff_drive(r1, 9, self.pose_left)
            self.diff_drive(r2, 9, self.pose_right)
            if self.counter == 200 / 5:
                # self.freeze(c4, 'adjust to parallel')
                self.phase = 4
                self.counter = 0
