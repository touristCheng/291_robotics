...
sub_round = 3
time_bound = 0.
# round_min_time = 16000.
# n = np.ceil(round_min_time / (20000. - round_min_time))
# global_thresh = (n + 1) * round_min_time
use_sub_bound = False
...
class Solution(SolutionBase):
    ...
    def init(self, env: FinalEnv):
        ...
        # self.round_min_time = 18000.
        # n = np.ceil(self.round_min_time / (20000. - self.round_min_time))
        # self.global_thresh = 200000. - (n + 1) * self.round_min_time

        self.box_info = {}
        for b_id in self.box_ids:
            self.box_info[b_id] = [0, 0]

        self.state_que = Queue(self.check_interval)


        global use_sub_bound, time_bound, sub_round
        if not use_sub_bound and global_time_left <= global_thresh:
            # print('use local time bound')
            use_sub_bound = True
            time_bound = np.floor(global_time_left * 1. / sub_round)
            # print('global left: ', global_time_left)
            # print('time bound: ', time_bound)

        
    def act(self, env: FinalEnv, current_timestep: int):

        global time_bound, use_sub_bound, global_time_left

        # if use_sub_bound:
        #     if (current_timestep // 5) > time_bound:
        #         # print('exeed local time bound.')
        #         return False
        ...
        # r1_state = r1.get_observation()[0]
        # r2_state = r2.get_observation()[0]
        # cur_state = np.concatenate([r1_state, r2_state], axis=0)
        # if self.state_que.full():
        #     prev_state = self.state_que.get()
        #     if np.allclose(prev_state, cur_state, ):
        #         return False
        #     else:
        #         self.state_que.put(cur_state)
        # else:
        #     self.state_que.put(cur_state)

        if self.phase == 0:
            if not use_sub_bound and 20000 - (current_timestep // 5) < 4100:
                return False
        ...
        if self.phase == 3:
            ...
            if self.counter < 500 / 5:
                ...
                if self.spade_is_empty(c4, r2):
                    # while not self.state_que.empty():
                    #     self.state_que.get()
                    self.phase = 0
                    global_time_left -= 1.
                    return True
...
def batch_test(seeds):
    ...
    for s in seeds:
        ...
        env = FinalEnv()
        save_dir = './demo_{:03d}'.format(s)
        mkdir_p(save_dir)
        # sys.stdout = open('{}/{:03d}_logs.txt'.format(save_dir, s), 'w+')
        env.run(Solution(), render=True, render_interval=50, save_dir=save_dir, save_render=False)
        env.close()

if __name__ == '__main__':

    # np.random.seed(int(time.time())%1000)
    # seed = np.random.random_integers(0, 1000, size=())
    # np.random.seed(seed)
    # env = FinalEnv()
    # # env.run(Solution(), render=True, render_interval=5, debug=True)
    # save_dir = './demo_{:03d}'.format(seed)
    # mkdir_p(save_dir)
    # sys.stdout = open('{}/{:03d}_logs.txt'.format(save_dir, seed), 'w+')
    # env.run(Solution(), render=False, render_interval=100, save_dir=save_dir)
    # # env.run(Solution())
    # env.close()
