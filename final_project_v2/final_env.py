...
class FinalEnv(Env):
    ...
    def run(self, solution: SolutionBase, render=False, render_interval=1, save_dir='', save_render=False, debug=False):
        """Run the environment
        ...
        while self.global_total_timesteps < self.global_max_steps:
            ...
            if save_render:
                ...
                save_path = '{}/{}'.format(save_dir, vid)
                mkdir_p(save_path)
                save_path = save_path + '/{:06d}.png'.format(fid)
                capture_image(save_path, c4)
            ...
            while True:
                ...
                if save_render and self.local_total_timesteps // self.frame_skip % render_interval == 0:
                    r1, r2, c1, c2, c3, c4 = self.get_agents()
                    save_path = '{}/{}'.format(save_dir, vid)
                    mkdir_p(save_path)
                    save_path = save_path + '/{:06d}.png'.format(fid)
...
def capture_image(save_path, c):
    color, depth, segmentation = c.get_observation()
    img = np.array(color*255, np.uint8)
